...
export default function Navbar() {
  ...
  return (
    <nav className="w-full p-4 bg-transparent  text-white flex justify-between items-center fixed top-0 left-0 right-0 z-50 ">
      ...
      <button className="hidden md:block px-4 py-2 border border-white rounded-lg text-sm uppercase tracking-widest hover:bg-white hover:text-black transition mr-6">
        Connect
      </button>
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-black bg-opacity-90 p-4 flex flex-col items-center space-y-4 md:hidden">
          ...
          <button
            className="px-4 py-2 border border-white rounded-lg text-sm uppercase tracking-widest hover:bg-white hover:text-black transition"
            onClick={() => setMenuOpen(false)}
          >
            Connect
          </button>
